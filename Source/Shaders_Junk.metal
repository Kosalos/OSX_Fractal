...
//    return 0.5 * log(r) * r/dr;
    ...
    return 0.3*abs(p.y)*length(p.xz) / p.w + step(1.0 + (1.0 - smoothstep(1.0, 5.0, abs(pos.x - control.camera.x)))*0.5, pos.y);

}

//// https://www.fractalforums.com/index.php?action=gallery;sa=view;id=18732
//float DE_MANDELBULB(float3 pos,device Control &control,thread float4 &orbitTrap) {
//    float r,theta,phi,re,im,a,x2;
//    float3 ot;
//
//    float dr = 1;
//    bool spin = false;
//    float3 origPos = pos * control.angle1;
//    float power1 = control.power - 1;
//    float twist = PI * control.bend;
//    float4 p = float4(pos,0.5);
//
//    for(int i=0; i < control.maxSteps; ++i) {
//        phi = atan2(pos.y,pos.x);
//        pos.x = cos(-phi) * pos.x - sin(-phi) * pos.y;
//        pos.y = 0;
//
...
//            im = sin(r);
//            a = re * pos.x - im * pos.y;
//            pos.y = re * pos.y + im * pos.x;
//            pos.x = a;
//        }
//
//        if(x2 * pos.x < 0) spin = !spin;
//
//        r = length_squared(pos);
//        r = pow(r,power1/2);
//        if(r > 4) break;
//
//        pos = pos * r + origPos;
//
//        r = length(pos);
//        dr = (pow(r, power1) * control.power * dr ) + 1.0;
//
//        ot = pos;
//        float4 hk = float4(ot,r);
//        orbitTrap = min(orbitTrap, dot(hk,hk));
...
/*
 View Profile
 March 01, 2016, 12:05:27 AMYes no problem, but i dont't think, that the formula is 'ready'. The overview of the 3D Mandelbrot form has still some ugly areas.
...
float4 DE_dual1(float3 pos,device Control &control,thread float4 &orbitTrap) {
    float k,t = control.foam2 + 0.25 * cos(control.bend * PI * control.multiplier * (pos.z - pos.x));
    float scale = 1;
 ...
 //   for(int i=0; i< control.maxSteps; ++i) {
  ...
  //  }
    ...
    return float4(pos,1.5 * (0.25 * abs(pos.y) / scale));
}

float4 DE_dual2(float3 pos,device Control &control,thread float4 &orbitTrap) {
    // For the Juliabox, c is a constant. For the Mandelbox, c is variable.
    float3 c = control.juliaboxMode ? control.julia : pos;
    float r2,dr = control.power;
    ...
    //}
        ...
        return float4(pos,length(pos)/abs(dr));
}


float DE_Dual(float3 pos,device Control &control,thread float4 &orbitTrap) {
...
//               }
//
//               r2 = length_squared(pos); //   pos.x*pos.x + pos.y*pos.y + pos.z*pos.z;
//
//               if(r2 < mR2) {
//                   float temp = fR2 / mR2;
//                   pos *= temp;
...
//float4 ans;
//    for(int i = 0; i < control.maxSteps; ++i) {
//        ans = DE_dual1(pos,control,orbitTrap);
//        pos = ans.xyz;
//    }
////    for(int i = 0; i < control.maxSteps/2; ++i) {
////        ans = DE_dual2(pos,control,orbitTrap);
////        pos = ans.xyz;
////    }
//
//    return ans.w;
...
float DE_KALIBOX(float3 pos,device Control &control,thread float4 &orbitTrap) {
    ...
    for (int i = 0; i < control.maxSteps;++i) {
        ...
        p *= clamp(max(control.cy/r2, control.cy), 0.0, 1.0);
        p *= control.mins;
...
float DE_Inner(float3 pos,device Control &control,thread float4 &orbitTrap) {
    return DE_Dual(pos,control,orbitTrap);
}
